use actix_web::{web, HttpRequest, HttpResponse, http::header::ContentType, get};
...
const DEFAULT_CONTENT_TYPE: HeaderValue = HeaderValue::from_static("application/octet-stream");

#[instrument(skip(req), fields(otel.kind=%SpanKind::Client, http.method="GET", http.target=%req.path(), http.user_agent=%req.headers().get("User-Agent").map(|h| h.to_str().unwrap_or("")).unwrap_or(""), http.status_code=field::Empty))]
#[get("/{path:.*}")]
pub async fn get_ui_path(req: HttpRequest) -> HttpResponse {
    match std::env::var("INTERFACE_BACKEND_URI").ok() {
        Some(uri) if !uri.is_empty() => {
            let client = reqwest::Client::new();
            let res = client.get(&format!("{}/{}", uri, req.match_info().query("path"))).send().await;
            match res {
                Ok(res) => {
                    let status = res.status();
                    let content_type = res.headers().get("content-type").unwrap_or(&DEFAULT_CONTENT_TYPE).clone();
                    Span::current()
